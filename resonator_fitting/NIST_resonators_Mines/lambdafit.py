...
def fit_lambda(I,f0,showplot=False):
    ...
    if showplot:
        ...
        tf0 = f0_of_I(ti,I0fit,Minfit,Mcfit,fbfit,lambfit)
        f0fit = f0_of_I(I,I0fit,Minfit,Mcfit,fbfit,lambfit)
        plt.figure(999)
        plt.subplot(2,1,1)
        plt.cla()
        plt.plot(I*1e6,(f0-fbfit)/1e3,'ob')
        plt.plot(ti*1e6,(tf0-fbfit)/1e3,'--r')
        ...
        plt.subplot(2,1,2)
        plt.cla()
        plt.plot(I*1e6,(f0 - f0_of_I(I,I0fit,Minfit,Mcfit,fbfit,lambfit))/1e3,'or')
        plt.xlabel('Current (uA)')
        plt.xlim(np.min(I)*1e6,np.max(I)*1e6)
