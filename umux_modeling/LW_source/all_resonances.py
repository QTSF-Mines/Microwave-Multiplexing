...
def fit_all_resonators(f, s21, tau, dist = 1.0e6, showplot = False, filename = None, condensed_plots_ids = None):
    # Keep the default 'dist' for the peak finder so it doesn't break
    peaks = find_all_peaks(f, s21, dist = dist, showplot = showplot, filename = filename)
    
    aas, taus, frs, Qrs, Qcs, Qis, phi0s, chi2s, bws = [], [], [], [], [], [], [], [], []

    # THE FIX: Slice by array indices, not Hz
    window_points = 50 

    for i in range(len(peaks)):
        peak_idx = peaks[i]
        
        # Safely slice to guarantee exactly 100 points
        start_idx = max(0, peak_idx - window_points)
        stop_idx = min(len(f), peak_idx + window_points)
        
        f_region = f[start_idx:stop_idx]
        s21_region = s21[start_idx:stop_idx]
        ...
        try:
            # Attempt the fit
            a, tau_fit, fr, Qr, Qc, phi0, chi2 = rf.refined_fit(f_region, s21_region, tau, plot_mode=plot_mode, filename = individual_filename)
            
            # If Qr or Qc are somehow returned as zero or None by a failed fit, skip the math
            if Qr is None or Qc is None or Qr == 0 or Qc == 0:
                continue

            bw = fr/Qr
            Qi = 1/(1/Qr - 1/Qc)
            
            # Only append successful data
            aas.append(a); taus.append(tau_fit); frs.append(fr); Qrs.append(Qr)
            Qcs.append(Qc); phi0s.append(phi0); chi2s.append(chi2)
            bws.append(bw); Qis.append(Qi)
            
        except Exception as e:
            # Silently skip catastrophic fits without breaking the loop
            pass
            
    if(showplot):
        # The plotting functions will now only receive clean data!
